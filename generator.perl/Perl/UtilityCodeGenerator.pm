...
void lock_perl_interpreter(bool lock);
...
void lock_perl_interpreter(bool lock) {
	if (lock) {
		while (hv_exists(PL_modglobal, "Haiku_interpreter_lock", 22)) {
			sleep(500);
		}
		hv_store(PL_modglobal, "Haiku_interpreter_lock", 22, &PL_sv_undef, 0);
		return;
	}
	else {
		hv_delete(PL_modglobal, "Haiku_interpreter_lock", 22, 0);
		return;
	}
}
